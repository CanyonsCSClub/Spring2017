...
public class PlayerControl : MonoBehaviour
{
	...
	void PlayerMovement()
	{
		...
		rb.MovePosition(transform.position + movement); // Adding player's input movement to the new position, and assigns the rigidbody a new position.
	}



	void PlayerRotation(float currentXRotation)
	{
		...
		transform.rotation = Quaternion.Euler(currentXRotation, 0, 0); // Transforms from the previous rotations to the new rotations.
	}

	public void changeRotation(float newYRot){
