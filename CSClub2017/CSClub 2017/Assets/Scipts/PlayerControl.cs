...
public class PlayerControl : MonoBehaviour
{
    ...
    public float speed = 18; // Declaring a variable that holds the player's speed.
    ...
    void Update() // Used a tutorial, hopefully going to be expanding on this method to suit the game's needs and functionalities.
    {
	...
	void FixedUpdate(){
		...
		transform.position += transform.forward * moveX + transform.forward * moveZ;
    ...
    void PlayerMovement()
    {
		...
		transform.position += transform.forward * (xAxis * zAxis); 
		//transform.position += (transform.forward + movement) * Time.deltaTime * speed;
    ...
    void PlayerRotation()
    {
        ...
        transform.rotation = Quaternion.Euler(currentRotationX, currentRotationY, 0); // Transforms from the previous rotations to the new rotations.
    }
